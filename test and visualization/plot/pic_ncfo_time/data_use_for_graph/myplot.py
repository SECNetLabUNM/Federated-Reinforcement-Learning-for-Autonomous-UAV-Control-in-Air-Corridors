...
for j, level_key in enumerate([20, 21]):
    for model_key, i in model_lst:
        ...
        axs[j].plot(agents_dic.keys(), won_rate_data, marker=dic_id_marker[model_key], label=dic_id_name[model_key],
                    color=dic_id_color[model_key])  # Call plot on axs, not fig
...
plt.legend()
fig.savefig('./pic_ncfo/test_2.jpg')
fig.savefig('./pic_ncfo/test_2.pdf')
plt.show()
...
plt.legend()
fig.tight_layout()
fig.savefig('./pic_ncfo/test_3.jpg')
fig.savefig('./pic_ncfo/test_3.pdf')
plt.show()
