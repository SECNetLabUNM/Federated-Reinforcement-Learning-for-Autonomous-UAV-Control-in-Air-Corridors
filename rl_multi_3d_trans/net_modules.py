...
class MAB(nn.Module):
    ...
    #     batch_size = q.size(0)
    #     q = self.split_heads(self.fc_q(q), batch_size)
    #     k = self.split_heads(self.fc_k(k_o), batch_size)
    #     v = self.split_heads(self.fc_v(k_o), batch_size)
    #     dk = q.size(-1)
    ...
    #
    #     concat_attention = scaled_attention.view(batch_size, -1, self.dim_V)
    #     attention_output = self.fc_o(concat_attention)
    #
    #     # Apply the first layer normalization after the attention mechanism
    #     attention_output = self.ln0(attention_output + q.view(batch_size, -1, self.dim_V))
    #
    #     # Apply the second layer normalization after the linear layer
    #     output = self.ln1(attention_output)
    #
    #
    #     return O
    #
    # def split_heads(self, x, batch_size):
    #     x = x.view(batch_size, -1, self.num_heads, 32)
    #     return x.permute(0, 2, 1, 3)
    #
    # def forward1(self, q, k_o,  mask=None):
    #     batch_size = q.size(0)
    #
    #     q = self.split_heads(self.fc_q(q), batch_size)
    #     k = self.split_heads(self.fc_k(k_o), batch_size)
