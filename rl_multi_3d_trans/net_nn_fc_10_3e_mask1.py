...
from rl_multi_3d_trans.net_modules import MAB,FcModule, BatchNormEmbedding
...
class SmallSetTransformer(nn.Module):
    def __init__(self, neighbor_dimension=7, net_width=256, with_position=False, token_query=False, num_enc=4,
        ...
        _encoder_layer = nn.TransformerEncoderLayer(d_model=net_width, nhead=4, dim_feedforward=net_width * 2,
                                                    batch_first=True)
        self.encoder = nn.TransformerEncoder(_encoder_layer, num_layers=num_enc)
        ...
        # _decoder_layer = nn.TransformerDecoderLayer(d_model=net_width, nhead=4, dim_feedforward=net_width * 2,
        #                                             batch_first=True)
        # self.decoder = nn.TransformerDecoder(_decoder_layer, num_layers=num_dec)
    ...
    def forward(self, x, uav, corridor, mask=None):
        x1 = self.encoder(x, src_key_padding_mask=mask)
        ...
        # x7 = self.decoder(tgt=query, memory=x1, memory_key_padding_mask=mask)
        for layer in self.decoder:
            query = layer.forward(query, x1, mask)
        x7 = query
...
class MergedModel(nn.Module):
    ...
    def forward(self, s1, s2=None):
        starts_with_1_0 = (s2[:, :, 0] == 1) & (s2[:, :, 1] == 0)
        # We create a mask of False where we do not want attention to be applied
        mask = torch.zeros_like(s2[:, :, 0], dtype=torch.bool)
        # Set entire rows to True in the mask where the condition matches
        #  positions with True are not allowed to attend while False values will be unchanged
        mask[starts_with_1_0] = True
        zero_col = torch.zeros_like(s1[:, 0:1], dtype=torch.bool)
        mask = torch.cat([zero_col, mask], axis=1)

        # mask=torch.zeros(s2.size()[0],s2.size()[1]+1)
        # mask[:,1:][starts_with_1_0] = True
        ...
        x = self.trans(s_p, uav=s1, corridor=c, mask=mask)
