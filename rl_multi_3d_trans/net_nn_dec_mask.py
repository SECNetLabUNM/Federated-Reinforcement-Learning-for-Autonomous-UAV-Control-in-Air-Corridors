...
from rl_multi_3d_trans.net_modules import FcModule, BatchNormEmbedding, MAB
...
class SmallSetTransformer(nn.Module):
    ...
    def forward(self, x, uav, corridor, mask):
        c = corridor.view(corridor.size(0), -1)
        state = torch.cat([uav, c], axis=1)

        query = self.eb(state)
        for layer in self.decoder:
            query = layer.forward(query, x, mask)
        x7 = query
        # x7 = self.decoder(tgt=query, memory=x, memory_key_padding_mask=mask)
...
class MergedModel(nn.Module):
    ...
    def forward(self, s1, s2=None):
        # starting with 10 meaning not avaliable
        ...
        # with s1_p in the k,v, performance is terrible, 0425
