...
class SmallSetTransformer(nn.Module):
    ...
    def forward(self, x, uav, corridor, closest):
        x1 = self.encoder(x)
        ...
        d = closest.view(closest.size(0), -1)
...
class MergedModel(nn.Module):
    ...
    def forward(self, s1, s2=None):
        ...
        arr = np.array(s2[:, :-4].to('cpu'))
        m, n, p = arr.shape
        results = []
        for i in range(m):
            valid_rows = [j for j in range(n) if arr[i, j, 0] == 0]
            if len(valid_rows) >= 2:
                # valid_rows.sort(key=lambda x: arr[i, :, 9])
                sorted_rows = sorted(valid_rows, key=lambda x: arr[i, x, 9])
                results.append(arr[i, sorted_rows[:2], :])
            elif len(valid_rows) == 1:
                tep = np.concatenate((arr[i, valid_rows[:1], :], np.zeros([1, 32])), axis=0)
                results.append(tep)
            else:
                results.append(np.zeros([2, 32]))
        new_arr = np.array(results,dtype=np.float32)
        other = torch.tensor(new_arr).to('cuda')

        x = self.trans(s_p, uav=s1, corridor=c, closest=other)
