...
class PPO(object):
    ...
    #         {key: torch.zeros_like(value) for key, value in models[0].state_dict().items() if value.requires_grad})
    #
    ...
    #     for model in models:
    #         full_state_dict = model.state_dict()  # Get the full original state
    #         for key, value in avg_state_dict.items():
    #             full_state_dict[key] = value  # Update only the trainable parameters
    ...
    def fed_average(self, key='actor'):
    ...
    def select_action(self, s1, s2, index):  # only used when interact with the env
        self.local_actors[index].eval()
        with torch.no_grad():
            s1 = np.array(s1)
            ...
            dist, alpha, beta = self.local_actors[index].get_dist(s1, s2)
    ...
    def evaluate(self, s1, s2, index, deterministic=True):
        ...
        with torch.no_grad():
            ...
            s2 = torch.FloatTensor(s2).to(device)
            if deterministic:
                action_with_highest_probability = self.local_actors[index].dist_mode(s1, s2)
                choosed_action = action_with_highest_probability
            else:
                dist, alpha, beta = self.local_actors[index].get_dist(s1, s2)
                choosed_action = dist.sample()
            return choosed_action.cpu().numpy(), 0.0
    ...
    def gae(self, unification=True):
        transitions = [[] for _ in range(self.num_agents)]
        collect_adv = [[] for _ in range(self.num_agents)]
        for agent in self.data:
            # name is also index
            agent_index = agent.name
            s1, s2, _, r, s1_prime, s2_prime, _, done_mask, dw_mask = self.make_batch(agent)
            ''' Use TD+GAE+LongTrajectory to compute Advantage and TD target'''
            self.local_critics[agent_index].eval()
            with torch.no_grad():
                vs = self.local_critics[agent_index](s1, s2)
                vs_ = self.local_critics[agent_index](s1_prime, s2_prime)
    ...
    def save(self, global_step, index=None):
        ...
        # for i, (local_actor, local_critic) in enumerate(zip(self.local_actors, self.local_critics)):
        #     torch.save(local_actor.state_dict(), f"{self.dir}/ppo_actor_{seq_name}_c{i}.pth")
        #     torch.save(local_critic.state_dict(), f"{self.dir}/ppo_critic_{seq_name}_c{i}.pth")
        ...
        torch.save(self.average_model_weights(self.local_critics, for_saving=True),
                   f"{self.dir}/ppo_critics_{seq_name}.pth")
    ...
    # def load_fed(self, folder, global_step):
    #     if isinstance(global_step, float) or isinstance(global_step, int):
    #         global_step = str(global_step / 1000000) + 'm'
    #     for i, (local_actor, local_critic) in enumerate(zip(self.local_actors, self.local_critics)):
    #         if folder.startswith('/'):
    #             saved_actor_weights = torch.load(f"{folder}/ppo_actor_{global_step}_c{i}.pth")
    #             saved_critic_weights = torch.load(f"{folder}/ppo_critic_{global_step}_c{i}.pth")
    #         else:
    #             saved_actor_weights = torch.load(f"./{folder}/ppo_actor_{global_step}_c{i}.pth")
    #             saved_critic_weights = torch.load(f"./{folder}/ppo_critic_{global_step}_c{i}.pth")
    #         local_actor.load_state_dict(saved_actor_weights, strict=False)
