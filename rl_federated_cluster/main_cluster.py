...
parser.add_argument('--num_obstacles', type=int, default=4, help='number of encoders')
...
def main():
    ...
    # else:
    if opt.current_time is None:
        run_name = f"d2move_{int(time.time())}_{exp_name[0]}"
    else:
        run_name = f"d2move_{opt.current_time}_{exp_name[0]}"
    ...
    print(f"{run_name}/process_log.txt")
    ...
    while total_steps < Max_train_steps:
        ...
        s, init_info = env.reset(seed=seed,
                                 ...
                                 turbulence_variance=opt.turbulence_variance)
        ...
        while env.agents:
            traj_lenth += 1
            steps += 1
            a_lst, logprob_a_lst, alpha, beta = [], [], [], []
            for agent, state in s1.items():
                ...
                if render:
                    env.render()
                    a_ele, logprob_a_ele = model.evaluate(s1_lst, s2_lst, index)
                else:
                    a_ele, logprob_a_ele, alpha_ele, beta_ele = model.select_action(s1_lst, s2_lst, index)
                    alpha += alpha_ele
                    beta += beta_ele
                a_lst.append(a_ele[0])
                logprob_a_lst.append(logprob_a_ele[0])
            ...
            if videoing:
                ...
                if turns == opt.video_turns:
                    ...
                    num_won = max(1, counter['won'])
                    ave_won_speed = ave_speed / num_won
                    ave_travel_time = ave_travel_time / num_won
            ...
            if ready_for_train and not env.agents:
                ...
                writer.add_scalar('weights/epoches', opt.K_epochs, total_steps)
                traj_lenth = 0
                episodes = 0
            total_steps += 1
