...
class PPO(object):
    def __init__(
        ...
        self.actor_optimizer = torch.optim.SGD(self.actor.parameters(), lr=a_lr)
        self.critic = net_models[net_model].CriticMulti(state_dim, s2_dim, net_width, shared_layers_critic).to(device)
        self.critic_optimizer = torch.optim.SGD(self.critic.parameters(), lr=c_lr)
    ...
    def train(self, global_step, epoches=None, anneal_mode='exponential'):
        ...
        lora.register_model_param_after_backward
    ...
    def gae(self, unification=True):
        # NOTE: see where GAE is used for method
        transitions = []
        collect_adv = []
        for agent in self.data:
            ...
            with torch.no_grad():
                ...
                for dlt, mask in zip(deltas[::-1], done_mask.cpu().flatten().numpy()[::-1]):
                    advantage = dlt + self.gamma * self.lambd * adv[-1] * (1 - mask) # important
