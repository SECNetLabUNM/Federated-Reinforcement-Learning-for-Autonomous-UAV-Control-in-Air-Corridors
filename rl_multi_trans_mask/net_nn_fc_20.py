...
class MergedModel(nn.Module):
    ...
    def eb_with_skip(self, x, embeder):
        y = torch.zeros((x.size(0), x.size(1), self.net_width), dtype=torch.float32, device=self.device)
        start_10 = (x[:, :, :2] == torch.tensor([0, 1], device=self.device)).all(dim=2)

        # Apply embedding to all matching rows at once
        # y[start_10] = embeder(x[start_10])
        for i in range(x.size(0)):
            # y[:, i, :][start_10[:, i]] = embeder(x[:, i, :][start_10[:, i]])
            print(x.size())
            y[i, start_10[i], :] = embeder(x[i, start_10[i], :]).view(-1, self.net_width)
        return y
