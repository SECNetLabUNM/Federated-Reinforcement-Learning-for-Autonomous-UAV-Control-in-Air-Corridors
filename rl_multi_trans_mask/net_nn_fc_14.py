...
class MergedModel(nn.Module):
    ...
    def forward(self, s1, s2=None):
        ...
        # observe 3 closest neighbors

        # s1_p = self.eb1(s1)
        s3 = s2[:, -4:]
        s2 = s2[:, :-4]

        sorted_tensor = torch.empty_like(s2)
        for i in range(s2.size(0)):
            rows_starting_with_01 = s2[i][:, 0] == 0.0
            rows_starting_with_01 &= s2[i][:, 1] == 1.0
            rows_starting_with_10 = s2[i][:, 0] == 1.0
            rows_starting_with_10 &= s2[i][:, 1] == 0.0
            rows_to_consider = s2[i][rows_starting_with_01]
            additional_rows = s2[i][rows_starting_with_10]

            # Sort these rows primarily by those starting with [0,1]
            _, sorted_indices = torch.sort(rows_to_consider[:, 9])  # Sorting by the third element for example
            sorted_rows = rows_to_consider[sorted_indices]

            # If we have fewer than 3 rows starting with [0,1], include [1,0]
            if sorted_rows.shape[0] < 3:
                needed = 3 - sorted_rows.shape[0]
                sorted_additional_rows = additional_rows[:needed]
                sorted_rows = torch.cat((sorted_rows, sorted_additional_rows), dim=0)


            # Replace the first 3 rows of the current batch with sorted rows
            s2[i][:3] = sorted_rows[:3]
        s2 = s2[:, :3, :]
        # s3_p = self.eb3(s3)
        ...
        other = s2.view(s2.size(0), -1)
        state = torch.cat([s1, c, other], axis=1)
